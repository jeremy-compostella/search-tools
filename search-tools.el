(defun search-do-mgrep (str)
  ...
  (let ((default-directory (concat current-root-path "/")))
...
(defun search-do-find-grep (str)
  (interactive (search-symbol-interactive "Find grep: "))
  (let ((default-directory (concat current-root-path "/")))
    (find-grep (format "find . -type f -exec grep -n -e '%s' /dev/null {} +" str)))
  str)

(defun search-do-find-file (filename)
  (interactive (search-symbol-interactive "Filename"))
  (find-name-dired current-root-path filename)
  filename)
